...
export default function SmartAssistant() {
  ...
  useEffect(() => {
    const loadInitial = async () => {
      try {
        ...
        if (serverSuggestions.length > 0 && lang === 'vi') setSuggestions(serverSuggestions);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  const suggestionIcon = (text) => {
    ...
    if (lower.includes('quy trình') || lower.includes('trạng thái') || lower.includes('register') || lower.includes('status')) return ClipboardList;
    if (lower.includes('giấy tờ') || lower.includes('bệnh viện') || lower.includes('document') || lower.includes('hospital')) return FileText;
    if (lower.includes('chuẩn bị') || lower.includes('ăn') || lower.includes('uống') || lower.includes('prepare') || lower.includes('eat') || lower.includes('drink')) return Utensils;
    if (lower.includes('lịch') || lower.includes('khi nào') || lower.includes('appointment') || lower.includes('when')) return Calendar;
    if (lower.includes('điều kiện') || lower.includes('sau khi hiến') || lower.includes('eligible') || lower.includes('after donation')) return HeartPulse;
    if (lower.includes('cần') || lower.includes('máu') || lower.includes('need') || lower.includes('blood')) return AlertTriangle;
    if (lower.includes('điểm') || lower.includes('huy hiệu') || lower.includes('point') || lower.includes('badge')) return Award;
    return Sparkles;
  };
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      <header className="rounded-3xl bg-gradient-to-r from-red-600 to-red-700 p-6 text-white shadow-lg shadow-red-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight">{tr('smartAssistantTitle')}</h1>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="flex min-h-[620px] flex-col assistant-card rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-6 py-4">
            <h2 className="text-lg font-black text-slate-900">{tr('dataDrivenChatbot')}</h2>
              ...
              return (
                <div key={idx} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
                  ...
                  >
                    <p>{msg.message}</p>
                    {msg.intent && !isUser && (
                      <p className="mt-2 text-[11px] font-bold uppercase tracking-widest text-slate-400">{tr('intent')}: {msg.intent}</p>
                    )}
                  </div>
                </div>
              );
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="flex items-center gap-2 rounded-3xl rounded-bl-lg border border-slate-100 bg-slate-50 dark:border-slate-700 dark:bg-slate-900 px-5 py-4 text-sm text-slate-500">
                  <Loader2 className="h-4 w-4 animate-spin" /> {tr('checkingData')}
                </div>
              </div>
            )}
            <div ref={bottomRef} />
        ...
        <aside className="space-y-4">
          <div className="assistant-card rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-black text-slate-900">{tr('quickQuestions')}</h3>
            </div>
