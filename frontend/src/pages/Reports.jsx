...
export default function Reports() {
  ...
  const exportFormat = getScopedSetting("export_format", "xlsx", user);
  ...
  const handleExportFile = async () => {
    try {
      ...
      if (exportStatus === "DONORS") {
        const blob = await analyticsService.exportDonors(isCSV);
        downloadFile(
          blob,
          `donor_list_${new Date().toISOString().slice(0, 10)}.${fileExt}`,
          mimeType,
        );
        return;
      }
      const blob = await analyticsService.exportTodayAppointments({
        status: exportStatus,
      ...
      downloadFile(
        blob,
        `appointments_${exportDate}_${exportStatus.toLowerCase()}.${fileExt}`,
        mimeType,
      );
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header>
        <div className="inline-flex items-center gap-2 rounded-full bg-green-50 px-3 py-1 text-xs font-black uppercase tracking-wider text-green-700 mb-3">
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_220px] gap-4 items-end">
          ...
          >
            <Download className="w-4 h-4" /> Xuất {exportFormat === 'csv' ? 'CSV' : 'Excel'}
          </button>
        </div>
      </section>
    </div>
