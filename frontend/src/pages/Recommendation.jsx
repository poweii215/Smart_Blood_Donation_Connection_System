      ...
      {weightSettings && <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <section className="recommendation-card bg-white rounded-3xl border border-gray-200 shadow-sm p-6">
          ...
          {recommendation && <div className="mt-5 space-y-3">
            ...
            {recommendation.items?.map((d, idx) => <div key={d.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4 dark:border-slate-700 dark:bg-slate-900">
              ...
              <p className="text-xs text-gray-500 mt-1">{d.phone} · {d.blood_type} · Reliability {Math.round((d.reliability_component || 0) * 100)}% · {tr('eligibleInDays', { days: d.days_until_eligible })}</p>
